...
import Tts from 'react-native-tts';           // ← default Android/iOS TTS
...
export default function ARVAScreen() {
  ...
  useEffect(() => {
    ...
    const stopListener   = Tts.addEventListener('tts-cancel',  () => setIsSpeaking(false));
    const errorListener  = Tts.addEventListener('tts-error',   () => setIsSpeaking(false));
  ...
  const speakReply = useCallback((text: string) => {
    ...
    setIsSpeaking(true);
    Tts.speak(clean); // ← default Android/iOS TTS engine
  }, []);

  const sendToARVA = useCallback(async (text: string) => {
    const trimmed = text.trim();
    if (!trimmed) return;
    if (isSpeaking) { Tts.stop(); setIsSpeaking(false); }
